...
void GalilControllerInterface::SetPositionMove(const prmMaskedDoubleVec &goalPosition) throw(ExcpSystemError, RobotException)
{
    ...
    if (goalPosition.Mask().Equal(false)){          //otherwise all axes on???
    ...
    //this puts it in velocity mode and pt mode...odd behavior
    //CreateCommandForAxis(buffer,"BG",goalPosition.Mask());   //start the motion (is this required?)
    //SendCommand(buffer);	
...
void GalilControllerInterface::SetVelocityMove(const prmMaskedDoubleVec &goalVelocity) throw(ExcpSystemError, RobotException)
{
    ...
    //correct for TM2500
    mtsDoubleVec v(goalVelocity.Data());
    //std::cerr << "Joint velocities being set by JG command:" << v <<std::endl;
    char buffer[G_SMALL_BUFFER];
    CreateCommand(buffer,"JG",goalVelocity.Mask(),v);
    SendCommand(buffer);
    ...
    vctBoolVec  startJogMask(goalVelocity.Mask());

    for( unsigned int  i =0 ; i <NumberActuators; i++) {
        if ( m_SoftRevLimitHit[i] ) {
            if(v[i] < 0)
                startJogMask[i] = false;
        }

        if ( m_SoftFwdLimitHit[i] ) {
            if(v[i] > 0)
                startJogMask[i] = false;
        }
     }
...
void GalilControllerInterface::SetAcceleration( const prmMaskedDoubleVec &acceleration) throw(ExcpSystemError, RobotException){
    ...
    mtsDoubleVec a(acceleration.Data());

    CreateCommand(buffer,"AC",acceleration.Mask(),a);
    SendCommand(buffer);
...
void GalilControllerInterface::SetDeceleration( const prmMaskedDoubleVec &deceleration) throw(ExcpSystemError, RobotException){
    ...
    mtsDoubleVec d(deceleration.Data());

    CreateCommand(buffer,"DC",deceleration.Mask(),d);
    SendCommand(buffer);
}
//    vctBoolVec  startJogMask(NumberActuators);
//    for( unsigned int  i =0 ; i <NumberActuators; i++) {
//        if (!IsMoving[i] && goalVelocity.Mask()[i])
//            startJogMask = true;
//        else
//            startJogMask = false;
//     }
//unsets the home variables at the same time.
void GalilControllerInterface::SetAbsolutePosition(const prmMaskedDoubleVec &position) throw(ExcpSystemError, RobotException){
    ...
    char buffer[G_SMALL_BUFFER];
    //Define Position
    CreateCommand(buffer,"DP",position.Mask(),position.Data());
    SendCommand(buffer);
}
